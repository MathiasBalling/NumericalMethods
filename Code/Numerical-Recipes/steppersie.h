...
template <class D> struct StepperSie : StepperBase {
  ...
  bool dy(VecDoub_I &y, const Doub htot, const Int k, VecDoub_O &yend, Int &ipt,
          VecDoub_I &scale, D &derivs);
...
StepperSie<D>::StepperSie(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx,
    ...
    : StepperBase(yy, dydxx, xx, atoll, rtoll, dens), nseq(IMAXX), cost(IMAXX),
      table(KMAXX, n), dfdy(n, n), dfdx(n), calcjac(false), a(n, n),
      coeff(IMAXX, IMAXX), fsave((IMAXX - 1) * (IMAXX + 1) / 2 + 2, n),
      dens((IMAXX + 2) * n), factrl(IMAXX) {
  static const Doub costfunc = 1.0, costjac = 5.0, costlu = 1.0,
                    costsolve = 1.0;
...
template <class D> void StepperSie<D>::step(const Doub htry, D &derivs) {
...
template <class D>
bool StepperSie<D>::dy(VecDoub_I &y, const Doub htot, const Int k,
                       VecDoub_O &yend, Int &ipt, VecDoub_I &scale, D &derivs) {
...
void StepperSie<D>::polyextr(const Int k, MatDoub_IO &table, VecDoub_IO &last) {
  ...
  for (Int j = k - 1; j > 0; j--)
    for (Int i = 0; i < l; i++)
      table[j - 1][i] =
          table[j][i] + coeff[k][j] * (table[j][i] - table[j - 1][i]);
  for (Int i = 0; i < l; i++)
    last[i] = table[0][i] + coeff[k][0] * (table[0][i] - last[i]);
}
template <class D>
void StepperSie<D>::prepare_dense(const Doub h, VecDoub_I &ysav,

...
struct Curve_interp {
  ...
  Curve_interp(MatDoub &ptsin, Bool close = 0)
      : n(ptsin.nrows()), dim(ptsin.ncols()), in(close ? 2 * n : n), cls(close),
        pts(dim, in), s(in), ans(dim), srp(dim) {
    Int i, ii, im, j, ofs;
    Doub ss, soff, db, de;
    ofs = close ? n / 2 : 0;
    s[0] = 0.;
  ...
  Doub fprime(Doub *x, Doub *y, Int pm) {
    ...
    return -(s1 * s2 / (s13 * s23 * s3)) * y[pm * 3] +
           (s1 * s3 / (s12 * s2 * s23)) * y[pm * 2] -
           (s2 * s3 / (s1 * s12 * s13)) * y[pm * 1] +
           (1. / s1 + 1. / s2 + 1. / s3) * y[0];
  }
